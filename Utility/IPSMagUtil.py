...
class Util(tk.Frame):
    ...
    def __init__(self, master):
        ...
        master.add(frame,text="IPS 120 controller")
        
        gbipLabel = tk.Label(frame,text="GBIP")
        gbipLabel.grid(column=0, row=0)
        self.gbipEntry = tk.Entry(frame,width = 10)
        self.gbipEntry.insert(tk.END,"25")
    ...
    def Connect(self):
        ...
        try:
            self.Mag = Inst.IPS120(rm,gbip)
            self.ConnStatusLabel["text"] = "Connected"
            self.ConnStatusLabel["bg"] = "green"
            print("Connected to IPS-120 magnet power supply")
            print("\n\n    WARNING: DO NOT START MEASURE WHILE\n    THIS IS STILL CONNECTED!!!   \n\n")
        except Exception as e:
            print(e)
            print("Failed to connect to IPS-120 magnet power supply")
            return False
    ...
    def GetStatusAndField(self):
        ...
        try:
            ...
            if self.Mag.is_SwitchHeaterOn:
                ...
                heaterStr = "OFF"
            # B, setB,Heater,Ramping,mode
            self.statParam = [B,setB,heaterStr,self.Mag.Ramping,self.Mag.Activity]
        except Exception as e:
            print(e)
            # B, setB,Heater,Ramping,mode
    ...
    def StartRamp(self):
        ...
        try:
            ...
            self.Mag.set_SetPoint(setB)
            time.sleep(0.1)
            self.Mag.set_FieldRate(rate)
            time.sleep(0.1)
            self.Mag.sweep_SetPoint()
            time.sleep(0.1)
